...
void ofApp::update() {
    ...
    if (maxColor == redCol) {
        ...
        if (redCol < greenCol) {
            explanationText = "SkiBlue-Like Color (Harmonic Colors)";
        }
        else {
    ...
    if (currentKey == '1') {
        ...
        explanationText = "\n Press ONE,TWO,THREE , L with mouse";
...
void ofApp::draw() {
    ...
    if (frameCount > 5 && frameCount <= 25) {
        ...
        for (int i = 0; i < 35; i++) {
            for (int j = 0; j < 30; j++) {
                ofSetColor(255);
                font.drawString("LETS START WITH IT", 0 + (230 * j), 0 + (25 * i));
            }
        }
    }
        ...
        ofSetColor(255);
        font.drawString("HARMONIC COLORS (RED, GREEN, BLUE) CREATE PLEASING INTERACTIONS'", ofGetWidth() / 2 - 185, 200);
        font.drawString("WHILE CUSTOM COLORS RESULT IN UNIQUE VISUAL EFFECTS.", ofGetWidth() / 2 - 165, 220);
    }
        ...
        ofSetColor(255);
        font.drawString("HOE WE CAN INTERACTE I HEAR YOU ASK??", ofGetWidth() / 2 - 150, 200);
    }
    else if (frameCount >= 2150 && frameCount <= 2550) {
        for (int i = 0; i < 35; i++) {
            ofSetColor(255);
            font.drawString("VELOCITY    ANGLE    POSTION    ROTATE    FORCE    SPINNING    HISTORY    RGB(CLOURE)    FRICTION", 140, 0 + (25 * i));
        }
    }
